...
const insertProduct = async (req, res) => {
  ...
  const product = await Products.findOne({ productname: productName, shopid: "SHOP001" });
  ...
  const barcodeId = (result.toString().slice(0,2) + productName.slice(0, 3) + "SHOP001".slice(0, 1)).trim();
...
const addOrder = async (req, res) => {
  try {
    ...
    const updated = await Products.updateOne({productname: productname}, {$set: {quantity: quantity}})
